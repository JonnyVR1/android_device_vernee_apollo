...
int main(int argc, char *argv[])
{
    ...
    int fwlog_stop_flag = 0;
    int dynamic_stop_flag = 0;
    int dump_retry = 0;
    ...
    while (!__io_canceled) {
...
#endif     
        if(fwlog_stop_flag == 0) {
            if (fwlogen_flag == 0 && fwlog_retry++ <= 15) {
                i_ret = property_get(WCN_FW_DBG_LOG_PROP, fwStateStr, NULL);
                if (0 >= i_ret) {
                    ALOGE("get property(%s) failed ret:%d\n", WCN_FW_DBG_LOG_PROP, i_ret);
                } else {
                    ALOGI("get property(%s) is %s by retry %d times\n", WCN_FW_DBG_LOG_PROP, fwStateStr, fwlog_retry);
                    if (strcmp(fwStateStr, fwStateStr0))
                        fwlogen_flag = 1;
                }
            }
            else if (fwlogen_flag == 1 || fwlog_retry == 16) {
                fwlog_stop_flag = 1;
                ALOGI("polling_flag:%d, retry: %d\n", fwlogen_flag, fwlog_retry);
        ...
        if (dynamic_stop_flag == 0) {
            if (dynamicdump_flag == 0 && dump_retry++ <= 15) {
                i_ret = property_get(WCN_DYNAMIC_DUMP_PROP, dynamicDump, NULL);
                if (0 >= i_ret)
                    ALOGI("get property(%s) failed ret:%d\n", WCN_DYNAMIC_DUMP_PROP, i_ret);
            else {
                    ALOGI("get property(%s) is %s by retry %d times\n", WCN_DYNAMIC_DUMP_PROP, dynamicDump, dump_retry);
                    if (strcmp(dynamicDump, dynamicDump0))
                        dynamicdump_flag = 1;
                }
            }
            else if (dynamicdump_flag == 1 || dump_retry == 16) {
                dynamic_stop_flag = 1;
